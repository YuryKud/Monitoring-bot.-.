Bot Monitoring Server
A server for monitoring and automatically restarting trading bots running on remote servers.

🚨 Purpose
This service solves a critical problem: it constantly checks whether your trading bots (Buy Bot and Sell Bot) are operational and automatically restarts them if they stop responding. This prevents downtime in trading activity, which could lead to financial losses.

🛠️ How it works
The system consists of two main components:

Flask REST API Server: Provides an HTTP endpoint for checking the current status of bots.
SSH Monitor: A background task that periodically checks whether bots are alive and manages their services.
Detailed monitoring mechanism ("Ping files")
This is the core of the entire system. A simple yet reliable mechanism based on "ping files" is used for monitoring:

Each trading bot (Buy Bot, Sell Bot) must periodically (e.g., every 30 seconds) update a special file on its server's disk. This update consists of writing the current Unix timestamp (time.time()) to the file.
The monitoring server connects to the remote server via SSH, reads the contents of these ping files, and compares the recorded time with the current time.
Decision logic:
Running: If the difference between the current time and the time in the file is less than PING_TIMEOUT (e.g., 120 seconds), the bot is considered running.
Not Running: If the difference is greater than PING_TIMEOUT, the bot is considered "frozen" or stopped. This bot's service is automatically restarted (systemctl restart ...).
File Not Found: If the ping file does not exist or is empty, the system will attempt to start the bot service (systemctl start ...), assuming it has been stopped. Architecture
+-------------------+ SSH (File Reading) +-----------------------+
| Monitoring Server | ----------------------------------> | Remote Server |
| (Flask API) | | - Buy Bot Service |
| - Status Endpoint | <--------------------------------- | - Sell Bot Service |
+-------------------+ (Ping files: timestamp) | - buy_bot.ping |
| - sell_bot.ping |
+-----------------------+
### Project Structure

bot-monitoring-server/ ├── monitoring_server.py # Main application script ├── .env.example # Example file with environment variables ├── requirements.txt # Python dependencies ├── bot_monitor.log # Log file (automatically generated) └── README.md

Setup and Installation
1. Dependencies
Make sure Python 3.8+ is installed. Install the required libraries: pip install -r requirements.txt

2. Setting up environment variables
Create a .env file in the project root directory based on the .env.example example:

Flask monitoring server configuration
MONITOR_SERVER_HOST=127.0.0.1 MONITOR_SERVER_PORT=5000

Connection parameters to the remote bot server
REMOTE_HOST=your-remote-server.com USERNAME_1=your-ssh-username PASSWORD=your-ssh-password

Names of your bot systemd services
BUY_BOT_SERVICE=buy_bot SELL_BOT_SERVICE=sell_bot

Absolute paths to ping files on the remote server
BUY_BOT_PING_FILE=/path/to/your/buy_bot.ping SELL_BOT_PING_FILE=/path/to/your/sell_bot.ping

Timeouts and intervals (in seconds)
PING_TIMEOUT=120 # A bot is considered "down" if it hasn't been pinged for longer than this amount of time. CHECK_INTERVAL=1800 # How often the monitor checks the bots' status (1800 seconds = 30 minutes).

Configuring Trading Bots Important! For monitoring to work, your bots must be implemented as systemd services and must constantly update their ping files.
Example code for a bot (Python):

python import time import os

PING_FILE = "/path/to/your/buy_bot.ping"

while True: # Your main trading logic here # ...

# Periodically update the ping file
with open(PING_FILE, 'w') as f:
f.write(str(time.time()))

time.sleep(30) # Update every 30 seconds
Example systemd service (/etc/systemd/system/buy_bot.service):

ini [Unit] Description=Buy Bot Service After=network.target

[Service] Type=simple User=ubuntu WorkingDirectory=/path/to/your/bot ExecStart=/usr/bin/python3 /path/to/your/bot/buy_bot.py Restart=on-failure RestartSec=10

[Install] WantedBy=multi-user.target 🚀 Startup Start the monitoring server:

bash python monitoring_server.py The server will start in two modes:

Background monitoring task: Checks bots every CHECK_INTERVAL seconds.

HTTP Server: Available at http://MONITOR_SERVER_HOST:MONITOR_SERVER_PORT.

📡 API Endpoints GET / Returns a welcome message.

GET /status Returns the current bot status in JSON format.

Example response:

json { "buy_bot": "Running", "sell_bot": "Not Running" } Possible status values:

"Running" - The bot is working properly.

"Not Running" - The bot is not responding, a restart has occurred.

"File Not Found" - The ping file was not found, the service has started.

"Error: ..." - An error occurred during verification.

"Unknown" - The status has not yet been determined (initial


# Bot Monitoring Server

Сервер для мониторинга и автоматического перезапуска торговых ботов, работающих на удаленных серверах.

## 🚨 Назначение

Этот сервис решает критически важную задачу: он постоянно проверяет, работают ли ваши торговые боты (Buy Bot и Sell Bot), и если они перестают отвечать — автоматически их перезапускает. Это предотвращает простои торговой активности, которые могут привести к финансовым потерям.

## 🛠️ Принцип работы

Система состоит из двух основных компонентов:

1.  **Flask REST API Сервер**: Предоставляет HTTP endpoint для проверки текущего статуса ботов.
2.  **SSH Монитор**: Фоновая задача, которая периодически проверяет "живы" ли боты, и управляет их сервисами.

### Детальный механизм мониторинга ("Ping-файлы")

Это ядро всей системы. Для мониторинга используется простой, но надежный механизм на основе "ping-файлов":

1.  **Каждый торговый бот** (Buy Bot, Sell Bot) должен периодически (например, каждые 30 секунд) обновлять специальный файл на диске своего сервера. Обновление заключается в записи в файл текущей Unix-метки времени (`time.time()`).
2.  **Мониторинговый сервер** по SSH подключается к удаленному серверу, читает содержимое этих ping-файлов и сравнивает записанное там время с текущим.
3.  **Логика принятия решения**:
    *   **`Running`**: Если разница между текущим временем и временем из файла меньше `PING_TIMEOUT` (например, 120 секунд), бот считается работающим.
    *   **`Not Running`**: Если разница больше `PING_TIMEOUT`, бот считается "зависшим" или остановившимся. Сервис этого бота автоматически перезапускается (`systemctl restart ...`).
    *   **`File Not Found`**: Если ping-файл не существует или пуст, система попытается запустить сервис бота (`systemctl start ...`), предполагая, что он был остановлен.

### Архитектура

```plaintext
+-------------------+        SSH (Чтение файлов)        +-----------------------+
| Monitoring Server | ----------------------------------> | Remote Server         |
| (Flask API)       |                                   | - Buy Bot Service     |
| - Status Endpoint | <--------------------------------- | - Sell Bot Service    |
+-------------------+        (Ping-файлы: timestamp)    | - buy_bot.ping        |
                                                         | - sell_bot.ping       |
                                                         +-----------------------+
### Структура проекта

```
bot-monitoring-server/
├── monitoring_server.py  # Основной скрипт приложения
├── .env.example          # Пример файла с переменными окружения
├── requirements.txt      # Зависимости Python
├── bot_monitor.log       # Файл логов (создается автоматически)
└── README.md

#### Настройка и установка
### 1. Зависимости
Убедитесь, что установлен Python 3.8+. Установите необходимые библиотеки:
pip install -r requirements.txt

### 2. Настройка переменных окружения

Создайте файл .env в корневой директории проекта на основе примера .env.example:

# Конфигурация Flask-сервера мониторинга
MONITOR_SERVER_HOST=127.0.0.1
MONITOR_SERVER_PORT=5000

# Параметры подключения к удаленному серверу с ботами
REMOTE_HOST=your-remote-server.com
USERNAME_1=your-ssh-username
PASSWORD=your-ssh-password

# Названия systemd сервисов ваших ботов
BUY_BOT_SERVICE=buy_bot
SELL_BOT_SERVICE=sell_bot

# Абсолютные пути к ping-файлам на удаленном сервере
BUY_BOT_PING_FILE=/path/to/your/buy_bot.ping
SELL_BOT_PING_FILE=/path/to/your/sell_bot.ping

# Таймауты и интервалы (в секундах)
PING_TIMEOUT=120    # Бот считается "упавшим", если не пинговался дольше этого времени
CHECK_INTERVAL=1800 # Как часто монитор проверяет состояние ботов (1800 сек = 30 мин)

3. Настройка Торговых Ботов
Важно! Для работы мониторинга ваши боты должны быть реализованы как systemd-сервисы и должны постоянно обновлять свои ping-файлы.

Пример кода для бота (Python):

python
import time
import os

PING_FILE = "/path/to/your/buy_bot.ping"

while True:
    # Ваша основная торговая логика здесь
    # ...

    # Периодическое обновление ping-файла
    with open(PING_FILE, 'w') as f:
        f.write(str(time.time()))
    
    time.sleep(30)  # Обновляем каждые 30 секунд
Пример systemd-сервиса (/etc/systemd/system/buy_bot.service):

ini
[Unit]
Description=Buy Bot Service
After=network.target

[Service]
Type=simple
User=ubuntu
WorkingDirectory=/path/to/your/bot
ExecStart=/usr/bin/python3 /path/to/your/bot/buy_bot.py
Restart=on-failure
RestartSec=10

[Install]
WantedBy=multi-user.target
🚀 Запуск
Запустите сервер мониторинга:

bash
python monitoring_server.py
Сервер начнет работать в двух режимах:

Фоновая задача мониторинга: Проверяет ботов каждые CHECK_INTERVAL секунд.

HTTP Сервер: Доступен по адресу http://MONITOR_SERVER_HOST:MONITOR_SERVER_PORT.

📡 API Endpoints
GET /
Возвращает приветственное сообщение.

GET /status
Возвращает текущий статус ботов в формате JSON.

Пример ответа:

json
{
  "buy_bot": "Running",
  "sell_bot": "Not Running"
}
Возможные значения статуса:

"Running" - Бот работает нормально.

"Not Running" - Бот не отвечает, выполнен перезапуск.

"File Not Found" - Ping-файл не найден, выполнен запуск сервиса.

"Error: ..." - Произошла ошибка при проверке.

"Unknown" - Статус еще не определен (начальное состояние).

📊 Логирование
Все события логируются в файл bot_monitor.log с указанием времени, уровня сообщения и деталей.

Пример лога:

text
2023-10-05 12:34:56,789 - INFO - Успешное подключение к серверу.
2023-10-05 12:34:57,123 - INFO - Начало проверки состояния ботов...
2023-10-05 12:34:57,456 - INFO - buy_bot работает нормально.
2023-10-05 12:34:57,789 - WARNING - sell_bot не работает. Перезапуск...
2023-10-05 12:34:58,123 - INFO - sell_bot успешно перезапущен.
🔧 Решение проблем
"Error: Authentication failed" в логах:

Проверьте USERNAME_1 и PASSWORD в файле .env.

Убедитесь, что на удаленном сервере разрешены SSH-подключения по паролю.

Статус бота постоянно "File Not Found":

Проверьте правильность путей BUY_BOT_PING_FILE и SELL_BOT_PING_FILE.

Убедитесь, что бот имеет права на запись в указанную директорию и действительно создает/обновляет файл.

Статус бота "Not Running", хотя он работает:

Увеличьте значение PING_TIMEOUT, если бот обновляет ping-файл реже, чем раз в PING_TIMEOUT секунд.

Проверьте, не "зависает" ли бот и не пропускает ли обновление ping-файла.

Flask-сервер не запускается:

Проверьте, не занят ли порт MONITOR_SERVER_PORT другим процессом.

🛡️ Безопасность
Никогда не коммитьте файл .env в Git! Он содержит пароли и чувствительные данные.

Для продакшн-среды рассмотрите использование SSH-ключей вместо паролей.

Ограничьте доступ к Flask-серверу (MONITOR_SERVER_HOST=127.0.0.1) или настройте брандмауэр.

🚀 Продвинутая настройка (Продакшн)
Запуск как systemd-сервис: Создайте сервис для самого мониторингового сервера, чтобы он автоматически запускался при загрузке системы и перезапускался при сбоях.

Docker: Оберните приложение в Docker-контейнер для простоты развертывания и управления.

Уведомления: Интегрируйте отправку уведомлений в Telegram/Slack при перезапуске бота.

